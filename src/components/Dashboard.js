...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoadinginit(true); // Set loading to true before starting fetches

      try {
        await Promise.all([fetchCompanyNames(), fetchRoleNames()]); // Wait for both functions to complete
      } catch (error) {
        console.error("Error fetching data:", error);
      } finally {
        setLoadinginit(false); // Set loading to false once both fetches are done
    ...
    fetchData(); // Start the data fetch process
  }, []);
